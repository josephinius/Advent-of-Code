...
long long lcm(vector<long long>& lens) {
    long long l = lens[0];
    for (int i = 1; i < lens.size(); i++) {
        l = lens[i] * l / gcd(lens[i], l);
    }
    return l;
}


long long find_length(Node* node, const string& instructions) {
    long long step = 0;
    while (node->name[2] != 'Z') {
...
int main() {
    ...
    for (auto node : pos) {
        lens.push_back(find_length(node, instructions));
        cout << find_length(node, instructions) << endl;
    }
    
    long long res = lcm(lens);
