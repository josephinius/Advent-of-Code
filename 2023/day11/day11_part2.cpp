...
int main() {
    ...
    vector<vector<char>> col_expanded(map.size(), vector<char>(map[0].size() + (expansion_factor - 1) * empty_cols.size(), '.'));
    ...
    vector<vector<char>> full_expanded(map.size() + (expansion_factor - 1) * empty_rows.size(), vector<char>(col_expanded[0].size(), '.'));
